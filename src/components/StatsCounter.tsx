        ...
        {stats.map((stat) => (
          <div key={stat.id} className="flex flex-col items-start gap-2 group">
            ...
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight">{stat.number}</span>
              <span className="text-3xl sm:text-4xl font-light text-[#2563eb]">{stat.symbol}</span>
            </div>
            <p className="text-xs sm:text-sm text-slate-600 font-medium leading-relaxed max-w-[200px]">{stat.label}</p>
          </div>
        ))}
      </div>
    </section>
